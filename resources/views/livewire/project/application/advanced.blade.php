<div>
    <div class="flex flex-col md:w-96">
        ...
        <div class="flex flex-col gap-1 pt-4">
            ...
            @endif
            <h3>AI</h3>
            <x-forms.checkbox instantSave id="isAiAutoFixEnabled" label="AI Auto-Fix"
                helper="Automatically analyze and fix deployment failures using AI. Fixes Docker-level issues in docker-compose.yml and Dockerfile."
                canGate="update" :canResource="$application" />
            @if ($application->git_based() && $application->is_github_based())
                <x-forms.checkbox instantSave id="isAiPushToGitEnabled" label="Push Fixes to Git"
                    helper="After AI fixes Docker files, automatically commit and push changes back to Git repository. Requires write access."
                    canGate="update" :canResource="$application" />
            @endif
            <h3>Security</h3>
            <x-forms.select label="Access List (IP Whitelist)" wire:model="accessListId" instantSave
                helper="Select an access list to restrict access to this application. IP whitelist is applied before SSL, so SSL certs can still be issued.">
                <option value="">None (Public)</option>
                @foreach ($accessLists as $list)
                    <option value="{{ $list['id'] }}">{{ $list['name'] }} ({{ count($list['ips']) }} IPs)</option>
                @endforeach
            </x-forms.select>
        </div>
