...
class GetLogs extends Component
{
    ...
    public function analyzeWithAi(): void
    {
        ...
        try {
            ...
            if (! $aiService->isConfigured()) {
                $this->ai_analysis_result = 'AI service not configured. Please set up your API key in Settings > AI Debug.';
            ...
            $this->ai_analysis_result = $aiService->analyzeLogs($logs, "container: {$this->container}");

            $this->dispatch('success', 'AI Analysis complete.');
        } catch (\Throwable $e) {
    ...
    {
        try {
            $this->authorize('view', $this->resource ?? $this->server);
            ...
            // For container logs, we need an Application to run auto-fix
            if (! $this->resource instanceof Application) {
                $this->dispatch('info', 'AI Auto-Fix', 'Auto-Fix is available for application deployments. Please use the deployment page to trigger auto-fix.');

                return;
            }

            // Find the most recent deployment for this application
            $deployment = ApplicationDeploymentQueue::where('application_id', $this->resource->id)
                ->orderBy('created_at', 'desc')
                ->first();

            if (! $deployment) {
                $this->dispatch('info', 'No Deployments Found', 'No deployment history found for this application.');

                return;
            }

            // Queue the auto-fix job
            AiAutoFixJob::dispatch($this->resource, $deployment);
